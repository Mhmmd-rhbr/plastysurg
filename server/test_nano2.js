...
async function test() {
  try {
    ...
    fs.writeFileSync('dummy4.png', dummyPng);
    const form = new FormData();
    form.append('file', fs.createReadStream('dummy4.png'));
...
Image URL: ${url}`;
    ...
    console.log(`Content:`, res.data.choices?.[0]?.message?.content?.substring(0, 500));
    console.log(`Output object:`, JSON.stringify(res.data._debug?.output?.[1] || res.data._debug?.output, null, 2));
  } catch (e) {
